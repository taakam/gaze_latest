...
# Fixed direct calibration:
# RealSense frame expressed in robot frame
T_RS_IN_ROBOT_PATH = "T_rs_in_robot.npy"
...
def main():
    ...
    # Load fixed direct RS -> robot transform
    T_rs_in_robot = np.load(T_RS_IN_ROBOT_PATH).astype(np.float32)
    print("[CALIB] Loaded fixed T_rs_in_robot from:", T_RS_IN_ROBOT_PATH)
    print(T_rs_in_robot)
    ...
    print("\nRunning live 3D fixation with FIXED rs->robot transform...")
    print("Press 'k' to send the current HIT_robot to Docker bridge")
            ...
            while True:
                ...
                if gaze_norm is not None and T_etg_in_rs is not None:
                    ...
                    if hit_rs is not None:
                        hit_robot = transform_point(T_rs_in_robot, hit_rs)
                ...
                if SHOW_WINDOWS:
                    ...
                    cv2.putText(
                        rs_vis,
                        "Press 'k' to send HIT_robot directly",
                        (20, 200),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.65,
                        (0, 200, 255),
                    ...
                    if key in (27, ord("q")):
                        ...
                        if hit_robot is not None:
                            ...
                            print("KEY 'k' PRESSED -> HIT_robot sent to Docker bridge")
                        else:
                            print("KEY 'k' PRESSED but no valid hit_robot available")
                ...
                if now - last_print_t > 0.5:
                    ...
                    if hit_rs is not None:
                        print(f"HIT_rs     xyz=[{hit_rs[0]:+.3f},{hit_rs[1]:+.3f},{hit_rs[2]:+.3f}]")
                    else:
                        print("HIT_rs: None")
