...
# -----------------------------
# Robot on stand
ROBOT_WORLD_XYZ = np.array([0.513099, -1.55089, 0.85], dtype=np.float32)

# Board moved closer to robot
BOARD_WORLD_XYZ = np.array([-0.05, -1.55089, 1.0175], dtype=np.float32)

# Hardcoded board origin expressed in robot base frame
# (robot yaw = pi, so do not compute with simple subtraction)
BOARD_IN_ROBOT = np.array([0.563, 0.0, 0.1675], dtype=np.float32)

# Safer hover while debugging
        ...
        with PupilStream(pupil_cfg) as pupil, RealSenseStream(rs_cfg) as rs_cam:
            while True:
                ...
                if board_out.get("ok", False) and hit_rs is not None:
                    ...
                    # safer clamps for moved-board setup
                    target_robot[0] = float(np.clip(target_robot[0], 0.40, 0.90))
                    target_robot[1] = float(np.clip(target_robot[1], -0.30, 0.30))
                    target_robot[2] = float(np.clip(target_robot[2], 0.18, 0.60))
